...
void add(int row, int col, int mat[row][col], int mat1[row][col], int result[row][col]) {
	int i, j;
	for(i = 0; i < row; i++) {
		for(j = 0; j < col; j++) {
...
void sub(int row, int col, int mat[row][col], int mat1[row][col], int result[row][col]) {
	int i, j;
	for(i = 0; i < row; i++) {
		for(j = 0; j < col; j++) {
			result[i][j] = mat1[i][j] - mat[i][j];
...
int main() {
	...
	add(row, column, mat, mat1, result);
	display(row, column, result);
	sub(row, column, mat, mat1, result);
	display(row, column, result);
